...
class UlusageDetVisionEmbeddings(nn.Module):
...
class UlusageDetVisionAttention(nn.Module):
...
class UlusageDetVisionEncoderLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = UlusageDetVisionAttention(config)
        self.input_layernorm = nn.LayerNorm(self.hidden_size, eps=config.layer_norm_eps)
        self.mlp = UlusageDetMLP(config)
...
class UlusageDetVisionEncoder(nn.Module):
    """
    Transformer encoder consisting of `config.num_hidden_layers` self attention layers. Each layer is a
    [`UlusageDetVisionEncoderLayer`].

    Args:
        config (`UlusageDetVisionConfig`):
            The corresponding vision configuration for the `UlusageDetEncoder`.
    """

    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([UlusageDetVisionEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class UlusageDetVisionModel(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embeddings = UlusageDetVisionEmbeddings(config)
        self.encoder = UlusageDetVisionEncoder(config)
...
class UlusageDocDetHReducerModel(PreTrainedModel):
...
class JapaneseUlusageDocDetHReducerModel(PreTrainedModel):
    def __init__(self, config, language_hidden_size):
        super().__init__(config)
        
        # 日本語文書用のレイアウト処理を追加
        self.vertical_text_processor = VerticalTextProcessor(config)
        self.ruby_text_processor = RubyTextProcessor(config)
        
        # 既存の処理を維持しつつ拡張
        self.ln_q = torch.nn.LayerNorm(self.config.hidden_size, eps=1e-6)
        self.conv_shape = (int(self.config.conv_shape.split('x')[0]), int(self.config.conv_shape.split('x')[1]))
        
        # 日本語特有のレイアウト特徴を処理するための追加レイヤー
        self.japanese_layout_processor = JapaneseLayoutProcessor(
